...
import { Check, ChevronDown, ImageDown, Loader2 } from 'lucide-react';
...
/**
 * Header action — download every screen in the current project as a ZIP of raw
 * PNGs (no bezel), foldered by the flow / sub-flow hierarchy. Mirrors
 * DownloadPdfButton: a version picker (defaults to "Latest"), agency-only via
 * the canEdit gate, enforced again server-side.
 */
export function DownloadPngsButton({
  appSlug,
  appName,
  builds,
}: {
  ...
  const onDownload = async (version: number | null): Promise<void> => {
    ...
    const filename = `${appSlug}-${version != null ? `v${version}` : 'latest'}-screens.zip`;
    const toastId = toast.start('zip', filename);
    try {
      const qs = version != null ? `?v=${version}` : '';
      const res = await fetch(
        `/api/projects/${encodeURIComponent(appSlug)}/download-pngs${qs}`,
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          disabled={pending}
          title={error ?? `Download ${appName} screens as PNGs (zip, no bezel)`}
          style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: 6,
        ...
        >
          ...
          <span>{pending ? 'Preparing…' : error ? 'Failed' : 'PNGs'}</span>
